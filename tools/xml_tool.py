...
from tools.specialCheck import *
...
class UIXMLTree:
    ...
    def process(self, xml_string, app_info=None, level=1, str_type="json", remove_system_bar=True, use_bounds=False,
        ...
        self.xml_dict = xmltodict.parse(etree.tostring(
            self.root, encoding='utf-8'), attr_prefix="")
    ...
    def get_xpath_new(self, node):
        ...
        while node is not None:
            ...
            if self.get_attr_count("tag", node.tag) == 1:
                ...
                array.append(
                    f'*[@content-desc="{node.attrib["content-desc"]}"]')
                break
            ...
            if self.should_remove_node(node):
                ...
                if node.attrib['class'] != "":
                    if node.attrib['text'] != "":
                        ...
                        node.attrib['name'] = node.attrib['class'].split(
                            '.')[-1]
                ...
                if node.attrib['class'] != "":
                    if node.attrib['class'].split('.')[-1] in ["View", "FrameLayout", "LinearLayout", "RelativeLayout"]:
                        ...
                        node.attrib['class'] = node.attrib['class'].split(
                            '.')[-1]
    ...
    def get_all_bounds(self, node, parent_keys):
        ...
        if 'bounds' in node.attrib:
            ...
            if parent_keys == []:
                self.all_bounds[key] = {
                    'bounds': node.attrib['bounds'], 'children': {}}
            else:
    ...
    def mid_order_reindex(self, node):
        ...
        if node.tag in self.node_to_xpath:
            ...
            self.node_to_xpath[node.tag] = [
                node.attrib['xpath1'], node.attrib['xpath2']]
        ...
        if node.getparent() is not None:
            ...
            if 'xpath1' in parent.attrib and 'xpath2' in parent.attrib:
                if parent.attrib['xpath1'] != "//" and parent.attrib['xpath2'] != "//":
                    if node.tag in self.node_to_xpath:
                        self.node_to_xpath[node.tag][2].append(
                            parent.attrib['xpath1'])
                        self.node_to_xpath[node.tag][2].append(
                            parent.attrib['xpath2'])
                    else:
                        self.node_to_xpath[node.tag][2] = [
                            parent.attrib['xpath1'], parent.attrib['xpath2']]
            # add sibling node
            children = list(parent)
            for _id, child in enumerate(children):
                if 'xpath1' in child.attrib and 'xpath2' in child.attrib:
                    if node.tag in self.node_to_xpath:
                        ...
                        self.node_to_xpath[node.tag][2] = [
                            child.attrib['xpath1'], child.attrib['xpath2']]
    ...
    def merge_description(self, p_desc, c_desc):
        p_list = p_desc.replace(";", " ").replace(
            ",", " ").replace(".", " ").split()
        c_list = c_desc.replace(";", " ").replace(
            ",", " ").replace(".", " ").split(";")
        candi_str = p_desc
    ...
    def dump_xpath(self):
        json_data = json.dumps(
            self.node_to_xpath, indent=4, ensure_ascii=False)
        print(json_data)
    ...
    def get_recycle_nodes(self, root):
        ...
        for element in root.iter():
            if 'scrollable' in element.attrib and element.attrib['scrollable'] == 'true':
                ...
                print(
                    element.attrib['class'], element.attrib['resource-id'], element.attrib['func-desc'])
        return node_list
