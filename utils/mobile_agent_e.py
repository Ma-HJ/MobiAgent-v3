...
class InfoPool:
    ...
    action_history: list = field(default_factory=list)  # List of actions in json
    ...
    last_summary: str = ""  # Last action description of Executor
    last_action: str = ""  # Last action str
    last_action_thought: str = ""  # Last action thought of Executor
    important_notes: str = ""
    ...
    # Planning
    plan: str = ""
    completed_plan: str = "" # completed_subgoal 与 history 结合
    progress_status: str = ""
    ...
    is_valid = True
    hint = ''
    changes = ''  # reflector's result
    related_info = ''  # the note
    wrong_actions = []  # based ion current interface, search the wrong actions
    elements = ""
    states = {}  # the state
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        if info_pool.plan == "":
            ...
            if info_pool.is_valid:
                prompt += "### Interface changes ###\n"
                prompt += f"After the action({info_pool.last_summary}) is performed, {info_pool.changes}. Analyzing the changes is helpful to plan the next step.\n\n"
            else:
                prompt += f"### Hint ###\n{info_pool.hint}\n\n"
            ...
            if info_pool.related_info != "":
                prompt += "### Related Information ###\n"
                prompt += f"{info_pool.related_info}\n\n"
            # prompt += f"""### Progress Status ###
            # You are in the process of finish the instruction, there are state you can reference:
            # {info_pool.states}\n\n
            # """
            ...
            # prompt += "Task-specific:\n"
            # if info_pool.additional_knowledge_manager != "":
            #     prompt += "You are in the process of performing the task. Past successful experiences should be used as reference, and incorrect methods should be avoided.\n"
            #     prompt += f"{info_pool.additional_knowledge_manager}\n\n"
            # else:
            #     prompt += f"{info_pool.add_info_token}\n\n"
...
class Executor(BaseAgent):
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        # prompt += f"### Some functional elements are as follows, you should analyze the current situation and decide next step based on the elements. \n{info_pool.elements}\n\n"
        # 是否可以给图片画框之后再给模型分析，但是这时的元素的性质就不可见了，或许画框时可以以不同的颜色标记，但是如果图片本身就有框呢
        # 当你想点击的时候，想要点击的元素一定在上述元素的范围之内，如果你想点击的元素没找到，就说明该元素不可点击，你应该分析你的思路错误并尝试其他操作
        # 如果一个元素的selected属性为true，说明该元素已选中，你可以进行下一步的操作
        # prompt += "### Functional Elements ###\nThere is an image that labels the elements in the picture, with different colors of the element boxes corresponding to different attributes.\n"
        # prompt += f"{info_pool.colors}\nWhen an element's attribute is 'hybrid', it means that the element can be both clicked and long-click.\n\n"

        if len(info_pool.wrong_actions) > 0:
            prompt += "### Wrong Action ###\nfollow action will result in wrong result, you should take other actions.\n"
            prompt += f"{info_pool.wrong_actions}\n\n"
        ...
        prompt += "3. If you want to open file manager, the name of app is 'Files by Google'.\n\n"
...
class ActionReflector(BaseAgent):
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        prompt += f"### Before Action Screenshot ###\n{info_pool.hint}"
        ...
        prompt += "### Changes ###\n"
        prompt += "Describe the changes before and after the interface(two attached images) after executing 'Latest Action', such as 'after clicking button, the number changed'."
    ...
    def parse_response(self, response: str) -> dict:
        ...
        outcome = response.split("### Outcome")[1].split("### Changes")[0].replace("\n", " ").replace("  ", " ").replace("###", "").strip()
        changes = response.split("### Changes")[1].split("### Error Description")[0].replace("\n", " ").replace("  ", " ").replace("###", "").strip()
        error_description = response.split("### Error Description")[-1].replace("\n", " ").replace("###", "").replace("  ", " ").strip()
        return {"outcome": outcome, "changes": changes, "error_description": error_description}
