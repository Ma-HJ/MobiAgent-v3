...
def run_instruction(adb_path, hdc_path, api_key, base_url, model, instruction, add_info, coor_type, if_notetaker,
                    max_step=25, log_path="./logs"):
...
{action_description}'''
            ...
            if action_object['action'] == "answer":
                ...
                message_data = {"name": "operator", "messages": message_operator, "response": operator_response,
                                "step_id": step + 1}
                with open(message_file, 'w', encoding='utf-8') as json_file:
                    json.dump(message_data, json_file, ensure_ascii=False, indent=4)
        ...
        message_data = {"name": "reflector", "messages": message_reflector, "response": output_action_reflect,
                        "step_id": step + 1}
        with open(message_file, 'w', encoding='utf-8') as json_file:
            json.dump(message_data, json_file, ensure_ascii=False, indent=4)
        ...
        if action_outcome == "A" and if_notetaker:
            ...
            message_data = {"name": "notekeeper", "messages": message_notekeeper, "response": output_note,
                            "step_id": step + 1}
            with open(message_file, 'w', encoding='utf-8') as json_file:
                json.dump(message_data, json_file, ensure_ascii=False, indent=4)
...
if __name__ == '__main__':
    ...
    run_instruction(args.adb_path, args.hdc_path, args.api_key, args.base_url, args.model, args.instruction,
                    args.add_info, args.coor_type, args.notetaker)
